...
; Recursión: No aplica
(define (pixbit? pixel)
  (if (and (= (length pixel) 3) (or (= (second pixel) 0) (= (second pixel) 1))) #t #f))
...
; Dominio: pixel
; Recorrido: int
; Descripción: Entrega el parámetro de color de un pixel de cualquier tipo
; Recursión: No aplica
(define (getColor pixel)
  (cond ((pixbit? pixel) (getBit pixel))
...
(define (flipH img)
  (define flipHpixel (lambda (pixel)
                       (list (- (getWidth img) (posX pixel)) (posY pixel) (getColor pixel) (getDepth pixel))))
  (list (getWidth img) (getHeight img) (map flipHpixel (getPixels img))))
...
; Recursión: No aplica
(define (flipV img)
  (list (getWidth img) (getHeight img) (flipVpixels (getPixels img) (- (getHeight img) 1))))
...
(define (flipVpixels pixlist height)
  (define flipVpixel (lambda (pixel)
                 (list (posX pixel) (- height (posY pixel)) (third pixel) (getDepth pixel))))
  (if (null? pixlist)
      null
      (cons (flipVpixel (car pixlist)) (flipVpixels (cdr pixlist) height))))
